...
func TestLoadParsesPluralProviderKeys(t *testing.T) {
	...
	t.Setenv("COPILOT_GITHUB_TOKENS", "copilot-one, copilot-two")
	...
	if got := cfg.AI.CopilotAPIKeys; len(got) != 2 || got[0] != "copilot-one" || got[1] != "copilot-two" {
		t.Fatalf("unexpected Copilot key list: %#v", got)
	}
...
func TestLoadFallsBackToSingularProviderKeys(t *testing.T) {
	...
	t.Setenv("COPILOT_GITHUB_TOKENS", "")
	t.Setenv("TAVILY_API_KEYS", "")
	t.Setenv("GEMINI_API_KEY", "gemini-single")
	t.Setenv("COPILOT_GITHUB_TOKEN", "copilot-single")
	...
	if got := cfg.AI.CopilotAPIKeys; len(got) != 1 || got[0] != "copilot-single" {
		t.Fatalf("unexpected Copilot key fallback: %#v", got)
	}
